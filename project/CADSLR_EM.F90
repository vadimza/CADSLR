    ...
    real*8,   allocatable, dimension(:)     :: Etot, Mtot, ecrs, mcrs
...
!c---------------------START LOOP OVER LAMBDA-----------------------------
    open(unit=70,file='x', status='unknown')
    open(unit=71,file='y', status='unknown')
    open(unit=72,file='z', status='unknown')
...
!c  
        allocate(Etot(1:Na),Mtot(1:Na),ecrs(1:Na),mcrs(1:Na))
        ...
        do i = 1,Na
            ...
            Etot(i) = rhs_ED_MD(   i,1) * zet_e(i) - E_field(i,1)
            Mtot(i) = rhs_ED_MD(Na+i,1) * zet_m(i) - H_field(i,1)
        
            do j = 1,Na
            
                ecrs(i) = ecrs(i) - C_tenzor(i,j) * rhs_ED_MD(Na+j,1)
                mcrs(i) = mcrs(i) + C_tenzor(i,j) * rhs_ED_MD(   j,1)
        ...
        do i = 1,N
            ...
            write(70,*) abs(ecrs(3*i-2))**2/abs(E_field(3*i-2,1))**2, abs(mcrs(3*i-2))**2/abs(H_field(3*i-1,1))**2
            write(71,*) abs(ecrs(3*i-1))**2/abs(E_field(3*i-2,1))**2, abs(mcrs(3*i-1))**2/abs(H_field(3*i-1,1))**2
            write(72,*) abs(ecrs(3*i  ))**2/abs(E_field(3*i-2,1))**2, abs(mcrs(3*i  ))**2/abs(H_field(3*i-1,1))**2
